...
from judge.models.assignment import Assignment
...
class Comment(MPTTModel):
    ...
    def most_recent(cls, user, n, batch=None):
        ...
        assignment_cache = CacheDict(lambda key: Assignment.objects.defer('description').get(key=key))
        ...
        assignment_cache = CahceDict(lambda key: assignment_cache[key].isaccessible_by(user))
        ...
        for i in itertools.count(0):
            ...
            for comment in slice:
                ...
                try:
                    ...
                    elif comment.page.startswith('a:'):
                        has_access = assignment_access[page_key]
                        comment.page_title = assignment_cache[page_key].name
    ...
    def link(self):
        try:
            ...
            elif self.page.startswith('a:'):
                line = reverse('assignment_view', args=(self.page[2:],))
    ...
    def get_page_title(cls, page):
        try:
            ...
            elif page.startswith('a:'):
                return Assignment.objects.values_list('name', flat=True).get(key=page[2:])
    ...
    def is_accessible_by(self, user):
        try:
            if self.page.startswith('p:'):
                ...
                return Assignment.objects.get(key=self.page[2:]).is_accessible_by(user)
            elif self.page.startswith('c:'):
