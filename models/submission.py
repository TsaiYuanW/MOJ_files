...
class Submission(models.Model):
    ...
    assignment_object = models.ForeignKey('Assignment', verbose_name=_('assignment'), null=True, blank=True,
                                       on_delete=models.SET_NULL, related_name='+')
    ...
    def can_see_detail(self, user):
        ...
        # If user is an author or curator of the assignment the submission was made in
        if self.assignment_object is not None and user.profile.id in self.assignment_object.editor_ids:
            return True
    ...
    def update_assignment(self):
        ...
        assignment.points = round(self.case_points / self.case_total * assignment_problem.points
                               if self.case_total > 0 else 0, 3)
        if not assignment_problem.partial and assignment.points != assignment_problem.points:
            assignment.points = 0
        assignment.save()
        assignment.participation.recompute_results()

    update_assignment.alters_data = True
    ...
    def assignment_key(self):
        if hasattr(self, 'assignment'):
            return self.assignment_object.key
