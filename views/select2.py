...
from judge.models import Comment, Contest, Organization, Problem, Profile, Assignment
...
class AssignmentSelect2View(Select2View):
    def get_queryset(self):
...
class AssignmentUserSearchSelect2View(userSearchSelect2View):
    def get_queryset(self):
        assignment = get_object_or_404(Assignment, key=self.kwargs['assignment'])
        if not assignment.is_accessible_by(self.request.user) or not assignment.can_see_full_scoreboared(self.request.user):
            raise Http404()
        
        return Profile.objects.filter(assignment_history__assignment=assignment,
                                      user__username_icontains=self.term).distinct()
