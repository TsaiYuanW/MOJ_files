...
from judge.models import Contest, Language, Problem, ProblemTranslation, Profile, Submission, Assignment
...
def submission_related(queryset):
    return queryset.select_related('user__user', 'problem', 'language') \
        .only('id', 'user__user__username', 'user__display_rank', 'user__rating', 'problem__name',
              ...
              'points', 'result', 'status', 'case_points', 'case_total', 'current_testcase', 'contest_object', 'assignment_object'
              'locked_after', 'problem__submission_source_visibility_mode') \
        .prefetch_related('contest_object__authors', 'contest_object__curators', 'assignment_object__authors', 'assignment_object__curators')
...
class SubmissionsListBase(DiggPaginatorMixin, TitleMixin, ListView):
    ...
    @cached_property
    def in_assignment(self):
        return self.request.user.is_authenticated and self.request.profile.current_assignment is not None
    ...
    @cached_property
    def assignment(self):
        return self.request.profile.current_assignment.assignment
    ...
    def _get_queryset(self):
        ...
        if self.in_contest:
        ...
        elif self.in_assignment:
            queryset = queryset.filter(assignment_object=self.assignment)
            if not self.assignment.can_see_full_scoreboard(self.request.user):
                queryset = queryset.filter(user=self.request.profile)
        else:
            queryset = queryset.select_related('contest_object').defer('contest_object__description')
            queryset = queryset.select_related('assignment_object').defer('assignment_object__description')
            ...
            if not self.request.user.has_perm('judge.see_private_contest'):
             ...
             if not self.request.user.has_perm('judge.see_private_assignment'):
                # Show submissions for any assignment you can edit or visible scoreboard
                assignment_queryset = Assignment.objects.filter(Q(authors=self.request.profile) |
                                                          Q(curators=self.request.profile) |
                                                          Q(scoreboard_visibility=Assignment.SCOREBOARD_VISIBLE) |
                                                          Q(end_time__lt=timezone.now())).distinct()
                queryset = queryset.filter(Q(user=self.request.profile) |
                                           Q(assignment_object__in=assignment_queryset) |
                                           Q(assignment_object__isnull=True))
    ...
    def get_context_data(self, **kwargs):
        ...
        context['dynamic_assignment_id'] = self.in_assignment and self.assignment.id
...
class ProblemSubmissionsBase(SubmissionsListBase):
    ...
    check_assignment_in_access_check = True

    def get_queryset(self):
        if self.in_contest and not self.contest.contest_problems.filter(problem_id=self.problem.id).exists():
            raise Http404()
        elif self.in_assignment and not self.assignment.assignment_problems.filter(problem_id=self.problem.id).exists();
            raise Http404()
    ...
    def access_check_assignment(self. request):
        if self.in_assignment and not self.assignment.can_see_own_scoreboard(request.user):
            raise Http404()
    ...
    def access_check(self, request):
        ...
        if self.in_assignment and request.user.is_authenticated and request.profile.id in self.assignment.editor_ids:
            return
        ...
        if self.check_assignment_in_access_check:
            self.access_check_assignment(request)
...
class UserProblemSubmissions(ConditionalUserTabMixin, UserMixin, ProblemSubmissions):
    ...
    check_assignment_in_access_check = False
...
class ForceAssignmentMixin(object):
    @property
    def in_assignment(self):
        return True
    ...
    def access_check(self, request):
        super(ForceAssignmentMixin, self).access_check(request)

        if not request.user.has_perm('judge.see_private_assignment'):
            if not self.assignment.is_visible:
                raise Http404()
            if self.assignment.start_time is not None and self.assignment.start_time > timezone.now():
                raise Http404()

    def get_problem_number(self, problem):
        return self.assignment.assignment_problems.select_related('problem').get(problem=problem).order

    def get(self, request, *args, **kwargs):
        if 'assignment' not in kwargs:
            raise ImproperlyConfigured(_('Must pass a assignment'))
        self._assignment = get_object_or_404(Assignment, key=kwargs['assignment'])
        return super(ForceAssignmentMixin, self).get(request, *args, **kwargs)
...
class UserAllAssignmnetSubmissions(ForceAssignmentMixin, AllUserSubmissions):
    def get_title(self):
        if self.is_own:
            return _('My submissions in %(assignment)s') % {'assignment': self.assignment.name}
        return _("%(user)s's submissions in %(assignment)s") % {
            'user': self.username,
    ...
    def access_check(self, request):
        super().access_check(request)
        if not self.assignment.users.filter(user_id=self.profile.id).exists():
            raise Http404()
        if not self.is_own and not self.assignment.can_see_full_scoreboard(self.request.user):
            raise Http404()

    def get_content_title(self):
        if self.is_own:
            return format_html(_('My submissions in <a href="{1}">{0}</a>'),
                               self.assignment.name, reverse("assignment_view", args=[self.assignment.key]))
        return format_html(_('<a href="{1}">{0}</a>\'s submissions in <a href="{3}">{2}</a>'),
                           self.username, reverse('user_page', args=[self.username]),
                           self.assignment.name, reverse('assignment_view', args=[self.assignment.key]))
    ...
    def get_queryset(self):
        queryset = super().get_queryset()
        # FIXME: fix this line of code when #1509 is implemented
        if not self.request.user.is_authenticated or self.request.profile.id not in self.assignment.editor_ids:
            filter_submissions_by_visible_problems(queryset, self.request.user)
        return queryset
...
class UserAssignmentSubmissions(ForceAssignmentMixin, UserProblemSubmissions):
    def get_title(self):
        if self.problem.is_accessible_by(self.request.user):
            return "%s's submissions for %s in %s" % (self.username, self.problem_name, self.assignment.name)
        return "%s's submissions for problem %s in %s" % (
            self.username, self.get_problem_number(self.problem), self.assignment.name)

    def access_check(self, request):
        super(UserAssignmentSubmissions, self).access_check(request)
        if not self.assignment.users.filter(user_id=self.profile.id).exists():
            raise Http404()
