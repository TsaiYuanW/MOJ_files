...
class TicketForm(forms.Form):
    ...
    def clean(self):
        if self.request is not None and self.request.user.is_authenticated:
            ...
            if not self.request.in_assignment and not profile.has_any_solves:
                raise ValidationError(_('You must solve at least one problem before you can create a ticket.'))
...
class NewProblemTicketView(ProblemMixin, TitleMixin, NewTicketView):
    ...
    def get_assignees(self):
        ...
        if self.request.in_assignment:
            assignment = self.request.participation.assignment
            if self.object.assignments.filter(assignment=assignment).exists():
                return assignment.authors.all()
