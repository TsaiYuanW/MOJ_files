...
class PostList(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            if participation:
                clarifications = ProblemClarification.objects.filter(problem__in=participation.assignment.problems.all())
                context['has_clarifications'] = clarifications.count() > 0
                context['clarifications'] = clarifications.order_by('-date')
        ...
        visible_assignments = Assignment.get_visible_assignments(self.request.user).filter(is_visible=True) \
                                  .order_by('start_time')

        context['current_assignments'] = visible_assignments.filter(start_time__lte=now, end_time__gt=now)
        context['future_assignments'] = visible_assignments.filter(start_time__gt=now)
