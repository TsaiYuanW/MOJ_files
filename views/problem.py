...
from judge.models import ContestSubmission, Judge, Language, Problem, ProblemGroup, \
    ...
    TranslatedProblemForeignKeyQuerySet, AssignmentSubmission
...
from judge.utils.problems import contest_attempted_ids, contest_completed_ids, hot_problems, user_attempted_ids, \
    user_completed_ids, assignment_attempted_ids, assignment_completed_ids
...
def get_assignment_problem(problem, profile):
    try:
        return problem.assignments.get(assignment_id=profile.current_assignment.assignment_id)
    except ObjectDoesNotExist:
        return None


def get_assignment_submission_count(problem, profile, virtual):
    return profile.current_assignment.submissions.exclude(submission__status__in=['IE']) \
...
class SolvedProblemMixin(object):
    def get_completed_problems(self):
        if self.in_assignment:
            return assignment_completed_ids(self.profile.current_assignment)
        else:
            return user_completed_ids(self.profile) if self.profile is not None else ()

    def get_attempted_problems(self):
        if self.in_assignment:
            return assignment_attempted_ids(self.profile.current_assignment)
        else:
            return user_attempted_ids(self.profile) if self.profile is not None else ()

    @cached_property
    def in_assignment(self):
        return self.profile is not None and self.profile.current_assignment is not None
...
class ProblemSolution(SolvedProblemMixin, ProblemMixin, TitleMixin, CommentedDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not solution.is_accessible_by(self.request.user) or self.request.in_contest:
            raise Http404()
        if not solution.is_accessible_by(self.request.user) or self.request.in_assignment:
            raise Http404()
...
class ProblemDetail(ProblemMixin, SolvedProblemMixin, CommentedDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        assignment_problem = (None if not authed or user.profile.current_assignment is None else
                           get_assignment_problem(self.object, user.profile))
        context['assignment_problem'] = assignment_problem
        if assignment_problem:
            clarifications = self.object.clarifications
            context['has_clarifications'] = clarifications.count() > 0
            context['clarifications'] = clarifications.order_by('-date')
            context['submission_limit'] = assignment_problem.max_submissions
            if assignment_problem.max_submissions:
                context['submissions_left'] = max(assignment_problem.max_submissions -
                                                  get_assignment_submission_count(self.object, user.profile,
                                                                               user.profile.current_assignment.virtual), 0)
...
class ProblemList(QueryStringSortMixin, TitleMixin, SolvedProblemMixin, ListView):
    ...
    def get_assignment_queryset(self):
        queryset = self.profile.current_assignment.assignment.assignment_problems.select_related('problem__group') \
            .defer('problem__description').order_by('problem__code') \
            .annotate(user_count=Count('submission__participation', distinct=True)) \
            .order_by('order')
        queryset = TranslatedProblemForeignKeyQuerySet.add_problem_i18n_name(queryset, 'i18n_name',
    ...
    def get_queryset(self):
        if self.in_contest:
            ...
            return self.get_assignment_queryset()
        else:
            return self.get_normal_queryset()

    def get_context_data(self, **kwargs):
        context = super(ProblemList, self).get_context_data(**kwargs)
        context['hide_solved'] = 0 if self.in_contest elif self.in_assignment else int(self.hide_solved)
        context['show_types'] = 0 if self.in_contest elif self.in_assignment else int(self.show_types)
        context['full_text'] = 0 if self.in_contest elif self.in_assignment else int(self.full_text)
        ...
        if not self.in_contest:
            ...
            context['hide_assignment_scoreboard'] = self.assignment.scoreboard_visibility in \
                (self.assignment.SCOREBOARD_AFTER_ASSIGNMENT, self.assignment.SCOREBOARD_AFTER_PARTICIPATION)
        return context
...
class ProblemSubmit(LoginRequiredMixin, ProblemMixin, TitleMixin, SingleObjectFormView):
    ...
    def assignment_problem(self):
        if self.request.profile.current_assignment is None:
            return None
        return get_assignment_problem(self.object, self.request.profile)
    ...
    def form_valid(self, form):
        ...
        with transaction.atomic():
            ...
            assignment_problem = self.assignment_problem
            if assignment_problem is not None:
                # Use the assignment object from current_assignment.assignment because we already use it
                # in profile.update_assignment().
                self.new_submission.assignment_object = self.request.profile.current_assignment.assignment
                if self.request.profile.current_assignment.live:
                    self.new_submission.locked_after = self.new_submission.assignment_object.locked_after
                self.new_submission.save()
                AssignmentSubmission(
                    submission=self.new_submission,
                    problem=assignment_problem,
                    participation=self.request.profile.current_assignment,
                ).save()
            else:
                self.new_submission.save()
