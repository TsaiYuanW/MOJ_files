...
from judge.views.submission import ForceContestMixin, ProblemSubmissions, ForceAssignmentMixin

__all__ = ['RankedSubmissions', 'ContestRankedSubmission', 'AssignmentRankedSubmission']
...
class RankedSubmissions(ProblemSubmissions):
    ...
    def get_queryset(self):
        ...
        if self.in_contest:
            ...
            assignment_join = '''INNER JOIN judge_assignmentsubmission AS cs ON (sub.id = cs.submission_id)
                              INNER JOIN judge_assignmentparticipation AS cp ON (cs.participation_id = cp.id)'''
        else:
            contest_join = ''
            assignment_join = ''
            points = 'sub.points'
        ...
        join_sql_subquery(
            ...
            subquery='''
                ...
                FROM (
                    SELECT sub.user_id AS uid, MAX(sub.points) AS points
                    FROM judge_submission AS sub {assignment_join}
                    WHERE sub.problem_id = %s AND {points} > 0 {constraint}
                    GROUP BY sub.user_id
                ) AS highscore STRAIGHT_JOIN (
                    SELECT sub.user_id AS uid, sub.points, MIN(sub.time) as time
                    ...
                    STRAIGHT_JOIN judge_submission AS sub
                        ON (sub.user_id = fastest.uid AND sub.time = fastest.time) {assignment_join}
                WHERE sub.problem_id = %s AND {points} > 0 {constraint}
                GROUP BY sub.user_id
            '''.format(points=points, assignment_join=assignment_join, constraint=constraint),
            params=params * 3, alias='best_subs', join_fields=[('id', 'id')],
        )
        ...
        if self.in_contest:
            ...
            return queryset.order_by('-assignment__points', 'time')
        else:
...
class AssignmentRankedSubmission(ForceAssignmentMixin, RankedSubmissions):
    def get_title(self):
        if self.problem.is_accessible_by(self.request.user):
            return _('Best solutions for %(problem)s in %(assignment)s') % {
                'problem': self.problem_name, 'assignment': self.assignment.name,
            }
        return _('Best solutions for problem %(number)s in %(assignment)s') % {
            'number': self.get_problem_number(self.problem), 'assignment': self.assignment.name,
        }
    ...
    def get_content_title(self):
        ...
        return format_html(_('Best solutions for problem {0} in <a href="{2}">{1}</a>'),
                           self.get_problem_number(self.problem), self.assignment.name,
                           reverse('assignment_view', args=[self.assignment.key]))

    def _get_queryset(self):
        return super()._get_queryset().filter(assignment_object=self.assignment)
