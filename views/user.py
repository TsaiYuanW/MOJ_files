...
from judge.utils.problems import contest_completed_ids, user_completed_ids, assignment_completed_ids
...
from .assignments import AssignmentRanking
...
class UserPage(TitleMixin, UserMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(self.object.ratins.aggregate(min_rating=Min('rating'), max_rating=Max('rating'),
...
class FixedAssignmentRanking(AssignmentRanking):
    assignment = None
...
def users(request):
    if request.user.is_authenticated:
        ...
        if participation is not None:
            ...
            return FixedAssignmentRanking.as_view(assignment=assignment)(request, assignment=assingment.key)
